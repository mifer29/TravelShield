...
open class UserReviewsViewModel : ViewModel() {
    ...
    open val userReview: StateFlow<Review?> get() = _userReview
    ...
    open fun fetchUserReviewForCountry(userId: String, countryName: String) {
        viewModelScope.launch {
            try {
                val snapshot = firestore.collection(REVIEWS_COLLECTION)
                    .whereEqualTo("userId", userId)
    ...
    open fun addReview(review: Review) {
        viewModelScope.launch {
            val userId = auth.currentUser?.uid
            if (userId == null) {
                _toastMessage.value = "User not authenticated."
    ...
    open fun updateReview(reviewId: String, newComment: String, newRating: Double) {
        viewModelScope.launch {
            try {
